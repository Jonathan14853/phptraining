...
function createCustomers($conn)
{
  ...
  if (empty($_POST["customer_number"])) {
    $error['customer_number'] = "Customer Number is required";
  } else {
...
if(!empty($customer_name) && !empty($customer_email) && !empty($customer_number) && !empty($gender))
{
$sql = "INSERT INTO customer (customer_name,customer_email,customer_number,gender)
VALUES ('$customer_name', '$customer_email','$customer_number','$gender')";
...
{
    mysqli_query($conn, "UPDATE customer WHERE id=$id");
    //$conn->close();
}
